...
<section class="search-form">
   <form action="" method="post">
      <input type="text" name="search" placeholder="Search Alert List..." class="box">
...
<!-- <h1>Teacher List</h1> -->
   ...
   <?php
      if(isset($_POST['submit'])){
         ...
         $result = mysqli_query($conn, "SELECT * FROM `user` WHERE fname LIKE '%{$search_item}%'") or die('query failed');
         if(mysqli_num_rows($result) > 0){
            while($fetch_product = mysqli_fetch_assoc($result)){
               
               $id = $fetch_product['user_id'];
              $fname = $fetch_product['fname'];
              $lname = $fetch_product['lname'];
              $mobileno = $fetch_product['phone'];
...
?>
   ...
   <form action="" method="post" class="box">
      <tr>
         ...
         <td><?php echo $fetch_product['fname']; ?></td>
         <td><?php echo $fetch_product['lname']; ?></td>
         <td><?php echo $fetch_product['phone']; ?></td>
         <td><?php echo $fetch_product['email']; ?></td>
         <td><?php echo $fetch_product['alert status']; ?></td>
